...
class ReducedOrderItem implements ModelInterface, ArrayAccess
{
    ...
    const FULFILMENT_STATUS_OPEN = 'OPEN';
    const FULFILMENT_STATUS_HANDLED = 'HANDLED';
     ...
     * @return string[]
     */
    public function getFulfilmentMethodAllowableValues()
    {
        return [
            self::FULFILMENT_METHOD_FBR,
            self::FULFILMENT_METHOD_FBB,
        ];
    }
    /**
     * Gets allowable values of the enum
     ...
     * @return string[]
     */
    public function getFulfilmentStatusAllowableValues()
    {
        return [
            self::FULFILMENT_STATUS_OPEN,
            self::FULFILMENT_STATUS_HANDLED,
        ];
    }
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getFulfilmentMethodAllowableValues();
        if (!is_null($this->container['fulfilment_method']) && !in_array($this->container['fulfilment_method'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'fulfilment_method', must be one of '%s'",
        ...
        $allowedValues = $this->getFulfilmentStatusAllowableValues();
        if (!is_null($this->container['fulfilment_status']) && !in_array($this->container['fulfilment_status'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'fulfilment_status', must be one of '%s'",
                implode("', '", $allowedValues)
            );
        }
    ...
    {
        $allowedValues = $this->getFulfilmentMethodAllowableValues();
        if (!in_array($fulfilment_method, $allowedValues, true)) {
     ...
     *
     * @param string $fulfilment_status This field indicates the processing state of an order. It can have two values: \"OPEN,\" which corresponds to active order items excluding those that have been shipped or cancelled, and \"HANDLED,\" which means the order item has either been shipped or cancelled.
     *
     * @return $this
     */
    ...
    {
        $allowedValues = $this->getFulfilmentStatusAllowableValues();
        if (!in_array($fulfilment_status, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'fulfilment_status', must be one of '%s'",
                    implode("', '", $allowedValues)
